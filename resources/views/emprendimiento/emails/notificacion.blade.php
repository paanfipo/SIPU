...
@section('title_email','Notificación Modulo Gestión')
@section('content')
                                                        ...
                                                        <tr>
                                                            <td style="word-wrap:break-word;font-size:0px;padding:10px 25px;padding-top:10px;padding-bottom:10px;padding-right:25px;padding-left:25px;" align="center">
                                                                <div class="" style="cursor:auto;color:#FFF;font-family:Helvetica;font-size:13px;line-height:22px;text-align:center;">
                                                                    ...
                                                                    <p> <strong>Conovocatoria</strong>: @if(isset($item)) {{$item["convocatoria"]}} @endif</p>
                                                                    <p> <strong>Etapa</strong>: @if(isset($item)) {{$item["etapa"]}} @endif</p>
                                                                        <p><strong>Actividad</strong>: @if(isset($item)) {{$item["actividad"]}} @endif </p>
                                                                        <p><strong>Cronograma</strong>: @if(isset($item)) {{$item["cronograma"]}} @endif</p>
                                                                        <p><strong>De</strong>: @if(isset($item)) {{$item["de"]}} @endif</p>
                                                                        <p><strong>Para</strong>: @if(isset($item)) {{$item["para"]}} @endif </p>
                                                                        @if(isset($item["url"]) && $item["url"] != "")
                                                                           <a href='{{route($item["url"], [ $item["convocatoria_id"], $item["usuario_id"] ])}}' target="_blank"> <p><strong>Mensaje</strong>: @if(isset($item)) {{$item["message"]}} @endif </p></a>
                                                                        @else
